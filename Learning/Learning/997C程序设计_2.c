...
一、选择题
	...
	B.从本程序文件的第一个函数开始, 到本程序文件的最后一个函数结束C.从 main()函数开始, 到main()函数结束
...
/*
1. 算法是一系列解决问题的清晰指令。与数学公式不同，算法更侧重于解决实际问题，需要具有可行性和实用性。

2. 算法可以用自然语言、流程图、N-S图、PAD图、伪代码等多种方式表示。

3. 语法错误是程序无法通过编译器的检查，例如拼写错误、缺少分号等；逻辑错误是程序可以通过编译但结果不符合预期，例如算法设计上的错误或者变量使用不当等。

4. C语言是一种面向过程的编程语言，其特点包括：语法简洁、运行效率高、灵活性强、可移植性好、可访问硬件资源等。

5. 该加法程序存在两个错误：缺少左花括号和加号应为加号而非减号。修改后的程序如下：
...
A.5.4321	B.5.43E2.l	C.e5 .4321	D. e6
2.能正确表示数学不等式0≤a≤10的C语言表达式是_
A.a> =0&a<= 10	B.a>=0lla<=10	C. a> =0&&a<= 10	D.0≤a&&as10
3．若a为 int型,且其值为3,则执行完表达式a+=8一=a* a后,a的值是_
A.-3	B.9	C.-12	D.6
