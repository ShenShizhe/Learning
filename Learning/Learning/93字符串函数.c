...
strlen函数的原型通常声明在string.h头文件中，其原型如下：

size_t strlen(const char *s);


其中，`const char *s`是一个指向字符串起始地址的指针，该函数返回参数`s`所指向的字符串的长度，
不包括字符串末尾的空字符'\0'。由于函数返回值类型为`size_t`，因此它能够处理非常长的字符串。
...
strcpy函数的原型通常声明在string.h头文件中，其原型如下：

char* strcpy(char* dest, const char* src);


其中，`dest`是要拷贝到的目标字符串指针，`src`是源字符串指针。该函数将源字符串(src)复制到目标字符串(dest)，
包括字符串末尾的空字符('\0')，返回值为`dest`指针。

需要注意的是，`dest`所指向的目标数组必须有足够的空间来存储`src`指向的字符串，否则可能会导致缓冲区溢出和未定义行为。
此外，如果`src`指向的字符串长度大于`dest`所指向的目标数组长度，则目标字符串将不会以'\0'结尾，这也会导致未定义行为。
...
strcat函数的原型通常声明在string.h头文件中，其原型如下：

char* strcat(char* dest, const char* src);

其中，`dest`是要追加字符串的目标字符串指针，`src`是要追加到目标字符串末尾的源字符串指针。
该函数将源字符串(src)追加到目标字符串(dest)的末尾，并返回`dest`指针。

需要注意的是，`dest`所指向的目标数组必须有足够的空间来存储`src`指向的字符串，否则可能会导致缓冲区溢出和未定义行为。
此外，如果`src`指向的字符串长度大于`dest`所指向的目标数组剩余空间，则目标字符串将不会以'\0'结尾，这也会导致未定义行为。
...
strcmp函数的原型通常声明在string.h头文件中，其原型如下：

int strcmp(const char* s1, const char* s2);

其中，`s1`和`s2`是要比较的两个字符串指针。该函数按照字典序比较两个字符串，并返回以下三种情况之一：

- 如果`s1`字符串小于`s2`字符串，则返回一个负整数。
- 如果`s1`字符串等于`s2`字符串，则返回0。
- 如果`s1`字符串大于`s2`字符串，则返回一个正整数。

需要注意的是，该函数区分大小写，即大写字母和小写字母被视为不同的字符。
...
strncpy函数的原型通常声明在string.h头文件中，其原型如下：

char* strncpy(char* dest, const char* src, size_t n);


其中，`dest`是要拷贝到的目标字符串指针，`src`是源字符串指针，`n`是要拷贝的字符数。
该函数将源字符串(src)的前n个字符复制到目标字符串(dest)，如果src的长度小于n，则剩余部分用空字符('\0')填充，返回值为`dest`指针。

需要注意的是，`dest`所指向的目标数组必须有足够的空间来存储`src`指向的字符串，否则可能会导致缓冲区溢出和未定义行为。
此外，如果`src`指向的字符串长度小于n，则目标字符串将不会以'\0'结尾，这也会导致未定义行为。

拷贝n个字符从源字符串到目标空间
如果源字符串的长度小于n，则拷贝完源字符串之后，在目标的后面追加'\0',直到n个
*/
#include <stdio.h>
#include <string.h>
...
strncat函数的原型通常声明在string.h头文件中，其原型如下：

char* strncat(char* dest, const char* src, size_t n);

其中，`dest`是要追加字符串的目标字符串指针，`src`是要追加到目标字符串末尾的源字符串指针，`n`是要追加的字符数。
该函数将源字符串(src)的前n个字符追加到目标字符串(dest)的末尾，并返回`dest`指针。

需要注意的是，`dest`所指向的目标数组必须有足够的空间来存储`src`指向的字符串和追加的字符，
否则可能会导致缓冲区溢出和未定义行为。此外，如果`src`指向的字符串长度小于n，则目标字符串将不会以'\0'结尾，这也会导致未定义行为。
*/
#include <stdio.h>
#include <string.h>
int main_93_06()
...
strerror函数的原型通常声明在string.h头文件中，其原型如下：

char* strerror(int errnum);


其中，`errnum`是一个整数参数，表示错误号。该函数返回指向错误消息字符串的指针，描述了`errnum`参数所代表的错误信息。

需要注意的是，`strerror`函数返回的错误消息字符串可能因平台而异。一些实现将该函数定义为可重入版本，即支持多线程，这样可以避免使用全局变量存储错误消息字符串。
char *strerror(int errnum);
